...
namespace WaveMetrixAssessment
{
        ...
        private static void ExecuteRequests()
        {
            ...
            while (_lowPriorityRequests.Count > 0 || _highPriorityRequests.Count > 0)
            {
                ...
                if (_highPriorityRequests.Count > 0)
                {
                    ...
                    request = _highPriorityRequests[0];
                    _highPriorityRequests.RemoveAt(0);
                }
                else
                {
                    priority = "low";
                    request = _lowPriorityRequests[0];
                    _lowPriorityRequests.RemoveAt(0);
                }

                var response = request.Client.GetAsync($"{request.Url}{request.AppId}").Result;
